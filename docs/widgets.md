...
`primary`, `success`, `warning`, `danger`, `info`, `pink`, `violet`, `secondary`, `dark`, `light`, `emerald`, and `sky`.

all colors are defined inside [Color.php](https://github.com/hawkiq/livewire-hwkUI/blob/main/src/Support/Color.php)
## 📦 Accordion
...
| Attribute | Type | Default | Description |
|---|---|---|---|
| `title` | mixed | `null` | Optional header title. |
| `icon` | mixed | `null` | Optional icon rendered in the header. |
| `theme` | `string/null` | `null` | Semantic color used by full, default-header, and outline modes. |
| `theme-mode` | `string/null` | `null` | `full` colors the whole card, `outline` applies a themed border, and any other value uses the default white card. |
| `header-class` | `string/null` | `null` | Additional classes for the header. |
| `body-class` | `string/null` | `null` | Additional classes for the body. |
| `footer-class` | `string/null` | `null` | Constructor property available to the component; the current view uses the footer slot classes generated by the component. |
| `disabled` | `boolean` | `false` | Displays the disabled overlay when truthy. |
| default slot | mixed | required | Card body. |
| `tools` slot | mixed | absent | Optional header tools. |
| `footer` slot | mixed | absent | Optional footer. |
...
| Front | Reveal |
|---|---|
|![Flip Card Widget](assets/flip-card-front-widget.png)|![Flip Card Widget](assets/flip-card-reveal-widget.png)|
...
| `value` | `int/float/string` | required | Display value. |
| `icon` | `string` | required | Icon name passed to `x-hwkui-icon`. |
| `href` | `string` | `#` | Link target. |
| `color` | `string` | `zinc` | Semantic color or supported legacy alias. Unknown values use the neutral zinc treatment. |
...
| Attribute | Type | Default | Description |
|---|---|---|---|
| `title` | mixed | `null` | Display title. |
| `text` | mixed | `null` | Main value. |
| `icon` | mixed | `null` | Icon name. |
| `description` | mixed | `null` | Supporting description. |
| `url` | mixed | `null` | Optional title link. |
| `url-target` | `string` | `_self` | Link target. |
| `theme` | `string/null` | `null` | Semantic box color. Without a theme, the box uses its neutral surface. |
| `icon-theme` | `string/null` | `null` | Semantic icon background color. Without it, the icon uses gray. |
| `progress` | `numeric/null` | `null` | Progress value clamped to `0` through `100`. |
| `progress-theme` | `string` | `white` | Semantic progress color. Unknown values use `bg-white`. |
...
| Attribute | Type | Default | Description |
|---|---|---|---|
| `title` | mixed | `null` | Main heading. |
| `text` | mixed | `null` | Supporting value. |
| `icon` | mixed | `null` | Decorative icon name. |
| `theme` | `string/null` | `null` | Semantic background color. Without a theme, the box uses its neutral surface. |
| `url` | mixed | `null` | Optional link URL. |
| `url-text` | mixed | `null` | Link label; the link is shown only when both `url` and `url-text` are present. |
| `url-icon` | mixed | `null` | Optional link icon name. |
| `loading` | `boolean/mixed` | `false` | Shows a loading overlay. |
...
![Tour Widget](assets/tour-widget.png)
